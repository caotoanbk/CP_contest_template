...
void solve(){
   ...
   grid.resize(r);
    ...
    FOR(i, sz(ms)){
        ...
        // dbg(src);
        vt<vt<bool>> used(r, vt<bool>(c, false));
        FOR(j, sz(rpos)){
            
           if(grid[rpos[j].fi][rpos[j].se] != 'R') continue; // skip removed robots
            auto [rr, rc] = rpos[j];
            pii npos = getNewPos(rr, rc, src.fi, src.se);
            dbg(npos);
            if(npos.fi == src.fi && npos.se == src.se){
                cout<<"kraj "<<i+1<<endl;
                return;
            }
            if(!used[npos.fi][npos.se]){
                used[npos.fi][npos.se] = true;
                grid[npos.fi][npos.se] = 'R'; 
                grid[rr][rc] = '.';
                dbg("one");
                rpos[j] = npos;
            }else{
                dbg("two");
                // grid[rr][rc] = '.';
                grid[npos.fi][npos.se] = '.';
                // rpos[j] = {-1, -1}; // mark as removed
            }
        }
        // FOR(j,r){
        //     FOR(k,c){
        //         if(grid[j][k] == 'X'){
        //             grid[j][k] = '.'; // remove 'X' marks
        //         }
        //     }
        // }
        dbg(grid);
