...
const int testcount = 1; //number of test
...
bool writeTest(int test) {
    ...
    for(int i=0; i<tc; i++){
        ...
        auto edges = genTree(n1);
        auto specialNodes = chooseSpecialNodes(n1, m1);
        ...
        int m2 = rnd.next(1,2);
        cout<<n2<<" "<<m2<<endl;
        auto edges2 = genTree(n2);
        auto specialNodes2 = chooseSpecialNodes(n2, m2);
        for (int x : specialNodes2) cout << x << " ";
        cout << endl;
        for (auto e : edges2) cout << e.first << " " << e.second << "\n";
        cout<< "\n";
    ...
    // system("sol_test.exe"); // run brute force
