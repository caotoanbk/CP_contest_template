...
void solve(){
    ...
    vt<int> freq;
    FOR(i, t){
        if(counts[i] >0) freq.pb(counts[i]);
    }
    ...
    FOR(i, t){
        int k = counts[i];
        FOR(j, b+1){
            if(dp[i][j] == 0) continue; // Skip if no ways to reach this state
            for(int x=0; x <= k && j+x <= b; x++){
                dp[i+1][j+x] = (dp[i+1][j+x] + dp[i][j]) % 1000000; // To avoid overflow
                // dp[i+1][j+x] %= 1000000; // To avoid overflow
            }
        }
    }
